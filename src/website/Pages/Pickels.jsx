...
function Pickels() {
    const [pickels, setpickels] = useState([]);
                ...
                axios
                  ...
                  .then((res) => {
                    const filtered = res.data.filter(
                      (item) => item.category === "Pickels"
                    );
                    setpickels(filtered);
                    setLoading(false);
                  })
                  .catch((err) => {
                    console.log("Error fetching pickels:", err);
                    setLoading(false);
                  });
              }, []);
    ...
    return ( 
      ...
      <div className="container">
        <div className="namkeens-header">
          <h2>Our Pickels</h2>
          <p>
            Explore our Spicy, tasty, and handcrafted pappads collection made
            with authentic flavors.
          </p>
        </div>

        {loading ? (
          <p className="loading-text">Loading Pickels...</p>
        ) : pickels.length === 0 ? (
          <p className="loading-text">No Pickels found.</p>
        ) : (
