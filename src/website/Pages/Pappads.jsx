...
function Pappads() {
    const [pappads, setpappads] = useState([]);
        ...
        axios
          ...
          .then((res) => {
            const filtered = res.data.filter(
              (item) => item.category === "Pappads"
            );
            setpappads(filtered);
            setLoading(false);
          })
          .catch((err) => {
            console.log("Error fetching pappads:", err);
            setLoading(false);
          });
      }, []);
    ...
    return ( 
      ...
      <div className="container">
        <div className="namkeens-header">
          <h2>Our Pappads</h2>
          <p>
            Explore our crunchy, tasty, and handcrafted pappads collection made
            with authentic flavors.
          </p>
        </div>

        {loading ? (
          <p className="loading-text">Loading Pappads...</p>
        ) : pappads.length === 0 ? (
          <p className="loading-text">No Pappads found.</p>
        ) : (
